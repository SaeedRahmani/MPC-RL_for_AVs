...
#     def plot(self):
#         """ Visualize the MPC solving process with buffer around ego vehicle """
...
#         self.ax.scatter(
#             x=[point[0] for point in self.reference_trajectory], 
#             y=[point[1] for point in self.reference_trajectory], 
#             color='grey', 
#             s=1,
#         )
...
#         # Plot buffer around the ego vehicle (as a rectangle)
#         buffer_distance = 2.6  # Adjust to the buffer size used in collision detection
#         buffer_rect = plt.Rectangle(
#             (ego_x - buffer_distance / 2, ego_y - buffer_distance / 2), 
#             buffer_distance, 
#             buffer_distance, 
#             linewidth=1, 
#             edgecolor='purple', 
...
#             linestyle='--'
#         )
#         self.ax.add_patch(buffer_rect)  # Add the buffer rectangle around ego vehicle
...
#         """ Collision detection with buffer around ego path, ensuring conflict points lie on the reference trajectory. """

#         # Create a buffered LineString for the ego vehicle's reference trajectory
#         ego_path_lineString = LineString([(point[0], point[1]) for point in self.reference_trajectory])
#         buffer_distance = 2.6  # Adjust buffer size as necessary
#         buffered_ego_path = ego_path_lineString.buffer(buffer_distance)
...
#             # Create a LineString from agent's current to future location
#             agent_path_lineString = LineString([agent_current_location, agent_future_location])

#             # Check if agent's path intersects the buffered ego path
#             intersection = buffered_ego_path.intersection(agent_path_lineString)
#             if not intersection.is_empty:
#                 # Find the nearest point on the reference trajectory within the buffer as the conflict location
#                 point_on_ego, _ = nearest_points(ego_path_lineString, intersection)
#                 conflict_location = np.array(point_on_ego.coords[0])
                
#                 # Find the index on the reference trajectory closest to this conflict point
#                 agent_index = np.argmin([np.linalg.norm(conflict_location - np.array(point[:2]))
#                                         for point in self.reference_trajectory])

#                 if ego_index < agent_index:  # Ensure conflict is ahead of ego
#                     # Calculate time-to-collision estimates (eta) for ego and agent
#                     agent_dist = np.linalg.norm(agent_current_location - conflict_location)
#                     agent_speed = agent_veh.speed
#                     agent_eta = agent_dist / agent_speed if agent_speed > 0 else np.inf

#                     # Calculate distance along reference trajectory from ego to conflict
#                     ego_dist = LineString([(point[0], point[1]) for point in self.reference_trajectory[ego_index:agent_index+1]]).length
#                     ego_eta = self.calculate_ego_eta(ego_dist, ego_index, self.last_acc, max_speed=10)

#                     # Collision condition based on time-to-collision threshold
#                     if np.abs(ego_eta - agent_eta) < self.ttc_threshold:
#                         print(f"Collision detected with eta difference: {np.abs(ego_eta - agent_eta)}")
#                         self.agent_collide.append(True)
#                         self.conflict_points.append((point_on_ego.x, point_on_ego.y))
...
#             points_for_linestring = [(point[0], point[1]) for point in self.reference_trajectory[self.ego_index: earliest_conflict_index+1]]

#             # Check if there are enough points to create a LineString
#             if len(points_for_linestring) > 1:
#                 distance_to_conflict = LineString(points_for_linestring).length
#             else:
