...
class IntersectionEnv(AbstractEnv):
        ...
        config.update(
            {
                ...
                "high_speed_reward": 1,
                "arrived_reward": 1,
                "reward_speed_range": [7.0, 9.0],  # Added back the missing parameter
                "normalize_reward": False,
                "offroad_terminal": False,

                "reward_weights": {
                "collision": -10.0,     # Large negative reward for collision
                "speed": 0.5,          # Moderate reward for maintaining speed
                "goal": 2.0,           # Good reward for reaching goal
                "comfort": -0.02,      # Small penalty for jerky movements
                "distance": 0.1        # Small reward for keeping safe distance
                },
                "min_safe_distance": 5.0,  # Minimum safe distance to other vehicles
                "comfort_acceleration": 3.0,  # Threshold for comfortable acceleration
                "target_speed": 10.0, 
                "normalize_reward": False,
                "offroad_terminal": False,
            }
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
        ...
        if vehicle.crashed:
            ...
            return reward  # Early return if crashed
        
        # 2. Speed reward - encourage maintaining reasonable speed
        target_speed = self.config["target_speed"]
        speed_reward = -abs(vehicle.speed - target_speed) / target_speed  # Normalized difference
        ...
        # 4. Comfort penalty (based on acceleration)
        if hasattr(vehicle, 'action'):  # Make sure action is available
            acceleration = abs(vehicle.action['acceleration'])
            if acceleration > self.config["comfort_acceleration"]:
                reward += weights["comfort"] * (acceleration - self.config["comfort_acceleration"])
        
        # 5. Distance reward (encourages keeping safe distance)
        ...
        if min_distance < self.config["min_safe_distance"]:
            ...
            distance_penalty = (self.config["min_safe_distance"] - min_distance) / self.config["min_safe_distance"]
            reward += weights["distance"] * (-distance_penalty)
    ...
    def _info(self, obs: np.ndarray, action: int) -> dict:
        ...
        # Add detailed reward components for analysis
        reward_info = {
            "collision": 0.0,
            "speed": 0.0,
            "goal": 0.0,
        ...
        if ego_vehicle.crashed:
            reward_info["collision"] = weights["collision"]
        
        speed_diff = abs(ego_vehicle.speed - self.config["target_speed"]) / self.config["target_speed"]
        reward_info["speed"] = weights["speed"] * -speed_diff
        
        if self.has_arrived(ego_vehicle):
            reward_info["goal"] = weights["goal"]
        
        if hasattr(ego_vehicle, 'action'):
            acceleration = abs(ego_vehicle.action['acceleration'])
            if acceleration > self.config["comfort_acceleration"]:
                reward_info["comfort"] = weights["comfort"] * (acceleration - self.config["comfort_acceleration"])
        
        min_distance = min(
            np.linalg.norm(ego_vehicle.position - other.position)
            for other in self.road.vehicles if other is not ego_vehicle
        )
        if min_distance < self.config["min_safe_distance"]:
            distance_penalty = (self.config["min_safe_distance"] - min_distance) / self.config["min_safe_distance"]
            reward_info["distance"] = weights["distance"] * (-distance_penalty)
    ...
    def _make_vehicles_original(self, n_vehicles: int = 10) -> None:
        """
