...
class IntersectionEnv(AbstractEnv):
        ...
        config.update(
            {
                ...
                #Added by me
                # "reward_weights": {
                #     "collision": -10.0,     # Large negative for crashes
                #     "speed": 1.0,           # Reward for maintaining good speed
                #     "arrival": 5.0,         # Reward for reaching goal
                #     "comfort": -0.02,       # Small penalty for jerky movements
                #     "distance": 0.1,        # Reward for keeping safe distance
                #     "on_road": 1.0          # Must stay on road
                # },
                ...
                "collision_reward": 0,
                "high_speed_reward": 1,
                "target_speed": 30.0,  # Desired target speed
                "arrived_reward": 1,
                "reward_speed_range": [7.0, 9.0],

                # # New reward parameters added by me 
                "min_safe_distance": 15.0,   # Minimum safe distance to other vehicles
                # "comfort_accel_range": 2.0, # Comfortable acceleration threshold
                # "target_speed": 10.0,       # Desired speed
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
        """Per-agent reward signal for speed and collision."""
        reward = 0.0

        # Collision penalty
        if vehicle.crashed:
            reward += -20 # Large negative reward for collision
        ...
        reward += distance_reward * 5  # Weight for distance reward
        # Speed reward (encourage maintaining a target speed)
        # target_speed = self.config.get("target_speed", 30.0)  # Default target speed
        # speed_diff = abs(vehicle.speed - target_speed)
        # speed_reward = -speed_diff / target_speed  # Normalize the penalty
        # reward += speed_reward * 1  # Weight for speed reward
        reward += vehicle.speed * 1
        return reward
